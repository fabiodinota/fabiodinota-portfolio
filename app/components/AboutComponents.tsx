...
interface AboutHeroProps {
	colors: { primary: string; secondary: string; background: string; border: string; red: string };
	theme: string;
	isOpenBig: boolean;
...
export function AboutHero({ colors, theme, isOpenBig, placeholderDark, placeholderLight }: AboutHeroProps) {
	return (
		<div
		...
		>
			<m.div
				...
				transition={{ duration: 1, delay: 0, ease: [0.200,0.005,0.000,0.995] as const }}
			...
			<m.div
				initial={{ opacity: 0, y: 50 }}
				animate={{ opacity: 1, y: 0 }}
				transition={{ duration: 1, delay: 0.1, ease: [0.200,0.005,0.000,0.995] as const }} 
...
export function AccordionSection({
	...
	return (
		<div
			...
			} transition-all duration-1000 custom-ease lg:hidden flex w-full flex-col justify-start items-center relative ${zIndex} top-0`}
		>
			<div
				className={`w-full absolute ${zIndex} top-0 left-0 flex items-center justify-between flex-shrink-0 flex-grow-0 h-[80px] px-[30px] ${borderClasses} ${colors.background} ${border} ${colors.primary}`}
			>
				<span>{title}</span>
				<span
					className={`${
